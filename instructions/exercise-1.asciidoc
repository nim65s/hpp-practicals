...
Before starting
---------------
HPP objects can be created and accessed direclty using boost python bindings.
...
Calls to method +problem.steeringMethod.steer+ create a new path between two configurations. This path is stored in a vector and can be accessed by the
index in the vector.
...
#   - "index of the path" means index in the vector of paths,
#
