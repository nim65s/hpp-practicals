...
\begin{frame} {Motions on Lie groups}
  \begin{itemize}
  \item $\conf\oplus\dconf\in\CS,\ \conf\in\CS, \dconf\in\TCS$, configuration reached from $\conf$ after following the constant velocity $\dconf$ during a unit time,
  \item $\conf_2\ominus\conf_1\in\TCS,\ \conf_1\in\CS, \conf_2\in\CS$, constant velocity that moves from $\conf_1$ to $\conf_2$ in unit time.
  \item $\conf_1 + t (\conf_2\ominus\conf_1)$ linear interpolation between $\conf_1$ and $\conf_2$.
  \end{itemize}
\end{frame}
